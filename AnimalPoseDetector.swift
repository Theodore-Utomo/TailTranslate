...
class AnimalPoseDetector: NSObject, ObservableObject {
    ...
    @Published var animalDetected: Bool?
    ...
    func detectedAnimalPose(request: VNRequest, error: Error?) {
        guard let animalBodyPoseResults = request.results as? [VNAnimalBodyPoseObservation],
              let animalBodyAllParts = try? animalBodyPoseResults.first?.recognizedPoints(.all),
              !animalBodyAllParts.isEmpty else {
            animalBodyParts = [:]
            animalDetected = false
            return
        }
        animalBodyParts = animalBodyAllParts
        animalDetected = true
    }
}
